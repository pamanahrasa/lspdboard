...
import { STATUS, todayISO, withComputedFields, formatDate, daysBetween, escapeHtml, OPEN_ENDED_LEAVE_TYPE } from './utils.js';
import { avatarChip, openEndedBadge } from './ui.js';
...
export function computeSummary(records, settings, today = todayISO()) {
    ...
    const awol = enriched.filter(r => r.status === STATUS.AWOL);
    ...
    // "Unavailable" = anyone not yet confirmed back on duty (Active, Returning
    // Today, Overdue, AWOL) - not just Active - so Available + On Leave always
    // sums back to the Total Roster instead of silently drifting apart.
    const onLeaveToday = active.length + returningToday.length + overdue.length + awol.length;
    const totalRoster = settings.totalRoster || 0;
    const officersAvailable = Math.max(0, totalRoster - onLeaveToday);

    return {
        enriched, active, upcoming, returningToday, returningTomorrow, startingTomorrow, overdue, awol, finished,
...
const CARD_DEFS = [
    ...
    { key: 'awol', label: 'AWOL', icon: 'fa-user-slash', tone: 'awol' },
    ...
    { key: 'onLeaveToday', label: 'Officers On Leave', icon: 'fa-user-group', tone: 'overdue' }
];
...
export function generateInsights(summary) {
    ...
    if (summary.awol.length) insights.push(`🚨 ${summary.awol.length} officer${summary.awol.length === 1 ? '' : 's'} ${summary.awol.length === 1 ? 'is' : 'are'} marked AWOL - return not confirmed, needs urgent Command follow-up.`);
    if (summary.overdue.length) insights.push(`⚠ ${summary.overdue.length} leave record${summary.overdue.length === 1 ? '' : 's'} ${summary.overdue.length === 1 ? 'is' : 'are'} overdue for return confirmation.`);
...
export function renderUpcomingLeave(container, records, today = todayISO(), graceDays) {
    ...
    container.innerHTML = enriched.length ? enriched.map(r => `
        <div class="flex items-center gap-3 py-2.5 border-b border-lspd-border last:border-0">
            ...
            <div class="min-w-0 flex-1">
                <p class="text-sm font-semibold text-lspd-text truncate">${escapeHtml(r.officerName)}</p>
                <p class="text-[11px] text-lspd-textSecondary truncate flex items-center">${escapeHtml(r.leaveType)}${r.leaveType === OPEN_ENDED_LEAVE_TYPE ? openEndedBadge() : ''}</p>
            </div>
