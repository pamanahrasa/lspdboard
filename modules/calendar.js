...
import { withComputedFields, todayISO, toISODate, formatDate, escapeHtml, monthLabel, STATUS_STYLES } from './utils.js';
import { openModal, statusBadge, avatarChip } from './ui.js';
        ...
        grid.innerHTML = cells.map(c => {
            ...
            <button data-iso="${c.iso}" class="cal-cell ${c.inMonth ? '' : 'cal-cell-outside'} ${c.isToday ? 'cal-cell-today' : ''}" style="background-color: rgb(var(--lspd-gold) / ${heat})">
                ...
                <span class="flex flex-col gap-0.5 w-full mt-1">
                    ${shown.map(r => `<span class="cal-chip ${STATUS_STYLES[r.status].bg} ${STATUS_STYLES[r.status].text}">${escapeHtml(r.officerName.split(' ')[0])}</span>`).join('')}
            ...
            <div class="space-y-1 max-h-96 overflow-y-auto pr-1">
                ...
                <button data-id="${r.id}" class="w-full flex items-center gap-3 py-2.5 px-2 rounded-lg hover:bg-lspd-navy/50 transition-colors text-left">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="text-[11px] text-lspd-textSecondary truncate">${escapeHtml(r.rank)} · ${escapeHtml(r.station)} · ${escapeHtml(r.leaveType)}</p>
                    </div>
