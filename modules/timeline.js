...
import {
    RANK_GROUPS, STATIONS, DIVISIONS, LEAVE_TYPES, STATUS_LIST, STATUS_STYLES, STATUS, OPEN_ENDED_LEAVE_TYPE,
...
export function initTimeline(container) {
    ...
    container.innerHTML = `
        ...
        <div class="timeline-legend">
            ...
            <span class="legend-chip"><span class="w-2.5 h-2.5 rounded-full bg-leave-openended inline-block"></span>Open-Ended Leave (dashed outline)</span>
        </div>
    ...
    function render() {
        ...
        const rows = officers.map(o => `
            <div class="timeline-row">
                ...
                <div class="timeline-track">
                    ${dayCells.join('')}
                    ${o.bars.map(b => `
                        <div class="timeline-bar ${STATUS_STYLES[b.record.status].dot} ${b.record.status === STATUS.AWOL ? 'awol-pulse' : ''} ${b.record.leaveType === OPEN_ENDED_LEAVE_TYPE ? 'openended-dashed' : ''}" style="--day-index:${b.dayIndex};--day-span:${b.span};" data-record-id="${b.record.id}" title="${escapeHtml(b.record.leaveType)} · ${b.record.status} · ${b.record.startDate} to ${b.record.endDate}"></div>`).join('')}
                </div>
            </div>`).join('');
        ...
        host.querySelectorAll('.timeline-name-cell[data-officer-name]').forEach(cell => {
            cell.addEventListener('click', () => {
                ...
                const primary = officerRecords.find(r => r.status === 'AWOL')
                    || officerRecords.find(r => ['Active', 'Overdue', 'Returning Today'].includes(r.status))
