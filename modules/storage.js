...
// Admin-only document (Firestore rules restrict both read AND write to
// authenticated users) - keeps the Settings-page PIN hash away from the
// public "settings" doc, which the Public Dashboard reads with no login.
const settingsPinRef = isConfigured ? doc(db, 'adminMeta', 'settingsPin') : null;
...
// ---------------------------------------------------------------------------
// Settings PIN (protects the Settings tab - see settings.js). Stored
// separately from the public "settings" doc so the hash is never readable
// without being signed in, unlike totalRoster/graceDays.
// ---------------------------------------------------------------------------

async function getSettingsPinHash() {
    requireConfigured();
    const snap = await getDoc(settingsPinRef);
    return snap.exists() ? snap.data().hash || null : null;
}

async function setSettingsPinHash(hash) {
    requireConfigured();
    await setDoc(settingsPinRef, { hash, updatedAt: new Date().toISOString() });
}
