...
import {
    RANK_GROUPS, STATIONS, DIVISIONS, LEAVE_TYPES, STATUS_LIST,
    withComputedFields, formatDate, escapeHtml, debounce, todayISO
} from './utils.js';
import { toast, openModal, closeModal, confirmDialog, statusBadge, avatarChip, renderFilterBar, applyFilters } from './ui.js';
...
export function renderLeaveTable(container, records, { editable = false, onView, onEdit, onDelete } = {}) {
    if (!records.length) {
    ...
    const rows = records.map(r => `
        <tr class="hover:bg-lspd-navy/40 cursor-pointer transition-colors" data-id="${r.id}">
            ...
            <td class="px-3 py-3 whitespace-nowrap">${escapeHtml(r.leaveType)}</td>
...
async function handleDelete(record) {
    ...
    if (!ok) return;
    StorageService.remove(record.id);
...
function formTemplate(r) {
    ...
    <form id="leave-form" novalidate>
        ...
        <div class="flex justify-end gap-3 mt-6">
            ...
            <button type="submit" class="btn-gold">${r ? 'Save Changes' : 'Add Leave Record'}</button>
        </div>
    </form>`;
}
...
export function openLeaveFormModal(existing) {
    ...
    $('#leave-form').addEventListener('submit', (e) => {
        ...
        if (existing) {
            StorageService.update(existing.id, payload);
...
export function openOfficerDetailModal(record, allEnrichedRecords) {
    ...
    const body = openModal({
        ...
        <div class="grid grid-cols-2 gap-x-4 gap-y-3 text-sm mb-5 p-4 rounded-lg bg-lspd-navy/50 border border-lspd-border">
            ...
            <div><p class="detail-label">Leave Type</p><p class="detail-value">${escapeHtml(record.leaveType)}</p></div>
        ...
        <div class="max-h-48 overflow-y-auto space-y-1.5 pr-1">
            ${history.map(h => `
                <div class="flex items-center justify-between text-xs py-1.5 px-2 rounded ${h.id === record.id ? 'bg-lspd-gold/10 border border-lspd-gold/30' : ''}">
                    <span class="text-lspd-textSecondary">${formatDate(h.startDate)} - ${formatDate(h.endDate)} · ${h.leaveType}</span>
