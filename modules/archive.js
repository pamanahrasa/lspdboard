...
import { RANK_GROUPS, STATIONS, DIVISIONS, LEAVE_TYPES, STATUS, withComputedFields, todayISO, debounce, arrayToCSV, downloadFile, escapeHtml } from './utils.js';
...
export function initArchive(container, { editable = false } = {}) {
    ...
    container.querySelector('#btn-export-archive').addEventListener('click', () => {
        const archive = currentArchive();
        if (!archive.length) { toast('No archived records to export.', 'error'); return; }
        const csv = arrayToCSV(archive, [
            ...
            { key: 'startDate', label: 'Start' }, { key: 'endDate', label: 'End' }, { key: 'duration', label: 'Duration (days)' },
            { key: 'reason', label: 'Reason' }, { key: 'notes', label: 'Notes' }
        ]);
        downloadFile('lspd-leave-archive.csv', csv, 'text/csv');
