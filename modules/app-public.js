...
import { StorageService, isConfigured } from './storage.js';
...
async function main() {
    applyStoredTheme();
    if (!isConfigured) document.getElementById('setup-notice').classList.remove('hidden');
    ...
    document.addEventListener('lspd:storage-error', (e) => {
        toast('Could not load live data: ' + (e.detail && e.detail.error && e.detail.error.message || 'connection issue.'), 'error');
    });
