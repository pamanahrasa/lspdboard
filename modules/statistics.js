...
import { withComputedFields, todayISO, readCssVar, LEAVE_TYPES } from './utils.js';
...
export function initStatistics(container) {
    ...
    function render() {
        ...
        makeChart(el('chart-type'), {
            type: 'doughnut',
            data: { labels: Object.keys(typeCounts), datasets: [{ data: Object.values(typeCounts), backgroundColor: PALETTE, borderColor: '#0f172a', borderWidth: 2 }] },
        ...
        makeChart(el('chart-duration'), {
            type: 'bar',
            data: { labels: LEAVE_TYPES, datasets: [{ label: 'Avg Days', data: avgDuration, backgroundColor: PALETTE[4], borderRadius: 6, maxBarThickness: 40 }] },
            options: { ...baseOptions(), indexAxis: 'y' }
        });
    }
