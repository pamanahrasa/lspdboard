 ...
 * settings.js
 * ---------------------------------------------------------------------------
 * Admin Settings: theme (local to this device), the two tunable numbers the
 * status engine depends on (Total Roster, grace days) which are shared and
 * synced to everyone via Firestore, and the Backup / Restore / Reset actions
 * (built on the same functions report.js uses for Export/Import). Reset and
 * Restore now affect the SHARED database that every device reads from, so
 * the copy here is explicit about that.
 * ---------------------------------------------------------------------------
 */
        ...
        container.innerHTML = `
            <div class="grid md:grid-cols-2 gap-5">
                ...
                <div class="lspd-card-panel">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="text-[11px] text-lspd-textSecondary mt-1">Used to calculate "Officers Available" (Roster - On Leave Today). Shared with everyone viewing the dashboard.</p>
                        </div>
                        <div>
                ...
                <div class="lspd-card-panel">
                    ...
                    <p class="text-[11px] text-lspd-textSecondary mt-3">Same JSON format as Reports &gt; Export/Import. Restoring replaces the <strong class="text-status-overdue">shared</strong> database everyone sees - export a backup first.</p>
                </div>
