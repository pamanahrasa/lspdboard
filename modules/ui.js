...
export function setTheme(theme) {
    document.documentElement.setAttribute('data-theme', theme);
    StorageService.updateSettings({ theme }); // routes to localStorage only - see storage.js
...
export function statusBadge(status, { withDot = true, size = 'sm' } = {}) {
    ...
    const pulse = status === 'AWOL' ? 'awol-pulse' : '';
    return `<span class="inline-flex items-center gap-1.5 rounded-full font-bold uppercase tracking-wide ${pad} ${s.bg} ${s.text} border ${s.border} ${pulse}">
        ${withDot ? `<span class="w-1.5 h-1.5 rounded-full ${s.dot}"></span>` : ''}${status}
    </span>`;
}

/** Small ∞ badge flagging a record as Open-Ended Leave - used next to the leave type wherever it's shown. */
export function openEndedBadge() {
    return `<span title="Open-Ended Leave - no fixed return date" class="inline-flex items-center justify-center w-4 h-4 rounded-full bg-leave-openended/20 text-leave-openended ml-1.5 shrink-0"><i class="fa-solid fa-infinity text-[9px]"></i></span>`;
}
